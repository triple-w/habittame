        ...
        if (isset($request['txref'])) {
            ...
            if ($resp['status'] = "success") {
                ...
                if ($resp['status'] = "success") {
                    ...
                    $user = $checkout->store($requestData, $transaction_id, $transaction_details);

                    $lastMemb = $user->memberships()->orderBy('id', 'DESC')->first();
                    $activation = Carbon::parse($lastMemb->start_date);
                    $expire = Carbon::parse($lastMemb->expire_date);
                    $file_name = $this->makeInvoice($requestData, "membership", $user, $amount, "Flutterwave", $requestData['phone'], $bs->base_currency_symbol_position, $bs->base_currency_symbol, $bs->base_currency_text, $transaction_id, $package->title, $lastMemb);
