...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
                            ...
                            <button class="btn btn-danger btn-sm float-right mr-2 d-none bulk-delete"
                                data-href="{{ route('admin.property_specification.bulk_delete_state') }}">
                                        ...
                                        <tbody>
                                            @foreach ($states as $state)
                                                <tr>
                                                    ...
                                                    @if ($settings->property_country_status == 1)
                                                        <td>
                                                            {{ strlen($state->country?->getContent($language->id)->name) > 50 ? mb_substr($state->country?->getContent($language->id)->name, 0, 50, 'UTF-8') . '...' : $state->country?->getContent($language->id)->name }}
                                                        </td>
                                                    @endif
                                                    <td>
                                                        ...
                                                        <div class="dropdown">
                                                            ...
                                                            data-{{ $lang->code }}_name="{{ $stat?->name }}" @endforeach>
                                                                ...
                                                                <form class="deleteForm d-inline-block dropdown-item"
                                                                    action="{{ route('admin.property_specification.delete_state') }}"
                                                                    method="post">
                                                                    @csrf
                                                                    <input type="hidden" name="id"
                                                                        value="{{ $state->id }}">

                                                                    <button type="submit" class=" deleteBtn p-0">
                                                                        <span class="btn-label">
    ...
    {{-- create modal --}}
    @include('backend.property.state.create')

    {{-- edit modal --}}
    @include('backend.property.state.edit')
@endsection
